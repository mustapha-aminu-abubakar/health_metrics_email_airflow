CREATE DATABASE IF NOT EXISTS health_metrics_3 DEFAULT CHARSET = 'utf8mb4';
USE health_metrics_3;
...
BEGIN
    WITH metrics_by_user_daily AS (
        ...
        WHERE `date` = (SELECT MAX(DATE(date_time)) FROM health_metrics_3.metrics)
    )
    SELECT * FROM metrics_latest LEFT JOIN health_metrics_3.users ON metrics_latest.user_id = users.user_id;
END $$
